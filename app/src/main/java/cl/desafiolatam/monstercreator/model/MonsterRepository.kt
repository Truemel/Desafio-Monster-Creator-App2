...
class MonsterRepository: MonsterRepositoryInterface {
    ...
    private val allMonsters: LiveData<List<Monster>>

    init {
        allMonsters = monsterDao.getAllMonsters()
    }
    ...
    override fun clearAllMonsters() {
        val creatureArray = allMonsters.value?.toTypedArray()
        if (creatureArray != null) {
            DeleteAsyncTask(monsterDao).execute(*creatureArray)
        }
    }


    private class InsertAsyncTask internal constructor(private val dao:  MonsterDao) : AsyncTask<Monster, Void, Void>() {
    ...
    private class DeleteAsyncTask internal constructor(private val dao: MonsterDao) : AsyncTask<Monster, Void, Void>() {
        override fun doInBackground(vararg params: Monster): Void? {
